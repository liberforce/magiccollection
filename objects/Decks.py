...
class Decks:
    ...
    def __init__(self, mainwindow):
        ...
        self.right_content_bot = Gtk.Box(
    ...
    def rename_deck(self, name_old, name_new):
        ...
        c_coll.execute(
            """UPDATE decks SET name = ? WHERE name = ?""",
            (
                name_new,
                name_old,
            ),
        ...
        c_coll.execute(
            """UPDATE collection SET deck = ? WHERE deck = ?""",
            (
                name_new,
                name_old,
            ),
        ...
        c_coll.execute(
            """UPDATE collection SET deck_side = ? WHERE deck_side = ?""",
            (
                name_new,
                name_old,
            ),
    ...
    def display_deck_content(
        self, selection, integer, TreeViewColumn, tree_editions, button_delete_deck
    ):
        ...
        if self.displaying_deck == 0:
            ...
            if treeiter != None:
                ...
                GLib.idle_add(
                    functions.decks.gen_deck_content,
                    deck_name,
                    self.right_content_bot,
                    self,
                )
            else:
    ...
    def add_cards_to_deck(self, deck_name, ids_coll_dict, side):
        ...
        for id_coll in ids_coll_dict.keys():
            c_coll.execute(
                """UPDATE collection SET """ + col_name + """ = ? WHERE id_coll = ?""",
                (
                    deck_name,
                    id_coll,
                ),
            ...
            if current_deck_name == deck_name:
                for i, card_data_deck in enumerate(self.mainstore):
                    for card_in_coll_dict in ids_coll_dict.values():
                        if (
                            card_data_deck[0] == card_in_coll_dict
                            and card_data_deck[16] == 0
                            and card_data_deck[17] == side
                        ):
                ...
                if len(c_to_add) > 0:
                    ...
                    reqq = (
                        """SELECT * FROM cards WHERE id IN (""" + id_list_req + """)"""
                    )
                    c.execute(reqq)
                    reponse_all = c.fetchall()
                    functions.db.disconnect_db(conn)

                    cards = functions.various.prepare_cards_data_for_treeview(
                        reponse_all
                    )
                    for card in cards.values():
                        ...
                        self.mainstore.insert_with_valuesv(
                            -1,
                            range(18),
                            [
                                card["id_"],
    ...
    def delete_cards_from_deck(self, deck_name, ids_coll_dict):
        ...
        for id_coll, data in ids_coll_dict.items():
            if data[1] == 0:
                c_coll.execute(
                    """UPDATE collection SET deck = ? WHERE id_coll = ?""",
                    (
                        "",
                        id_coll,
                    ),
                ...
                c_coll.execute(
                    """UPDATE collection SET deck_side = ? WHERE id_coll = ?""",
                    (
                        "",
                        id_coll,
                    ),
        ...
        ids_list = ids_list[:-2]
        c_coll.execute(
            """SELECT id_coll, id_card, deck, deck_side FROM collection WHERE id_card IN ("""
            + ids_list
            + """) AND (deck != \"\" OR deck_side != \"\")"""
        )
        ...
        for i, row in enumerate(coll_object.mainstore):
            if (
                row[0] not in dict_responses_coll.keys()
                and row[0] in ids_coll_dict_ids_card_list
            ):
                coll_object.mainstore[i][13] = Pango.Style.NORMAL
                if row[0] in defs.SDF_RECTO_IDS_LIST:
                    to_unitalic_after_in_search.append(
                        defs.SDF_RECTO_VERSO_IDS_DICT[row[0]]
                    )
        ...
        if coll_object.tree_coll.get_model() == coll_object.searchstore:
            for i, row in enumerate(coll_object.searchstore):
                if (
                    row[0] not in dict_responses_coll.keys()
                    and row[0] in ids_coll_dict_ids_card_list
                ) or (row[0] in to_unitalic_after_in_search):
    ...
    def change_nb_proxies(self, deck_name, proxies_list_to_change):
        ...
        for data in proxies_list_to_change:
            ...
            if side == 0:
                ...
                try:
                    dict_current_proxies_side[id_db] = dict_current_proxies_side[
                        id_db
                    ] + int(qnt)
                except KeyError:
                    if int(qnt) > 0:
        ...
        if change_proxies > 0:
            ...
            c_coll.execute(
                """UPDATE decks SET proxies = ? WHERE name = ?""",
                (
                    proxies_str,
                    deck_name,
                ),
        ...
        if change_proxies_side > 0:
            ...
            proxies_str = proxies_str[:-3]
            c_coll.execute(
                """UPDATE decks SET proxies_side = ? WHERE name = ?""",
                (
                    proxies_str,
                    deck_name,
                ),
    ...
    def move_row(self, old_deck, new_deck, ids_db_list):
        ...
        for data in ids_db_list:
            ...
            if side == 0:
                c_coll.execute(
                    """UPDATE collection SET deck = ? WHERE id_card = ? AND deck = ?""",
                    (
                        new_deck,
                        id_db,
                        old_deck,
                ...
                c_coll.execute(
                    """UPDATE collection SET deck_side = ? WHERE id_card = ? AND deck_side = ?""",
                    (
                        new_deck,
                        id_db,
                        old_deck,
        ...
        for row in ids_db_list:
            ...
            if current_side == 0:
                if proxy == 0:
                    ...
                    c_coll.execute(
                        """SELECT id_coll, deck, deck_side FROM collection WHERE (deck = ? OR deck_side = ?) AND id_card = ?""",
                        (
                            deck_name,
                            deck_name,
                            id_db,
                    ...
                    for id_coll in ids_coll_list:
                        c_coll.execute(
                            """UPDATE collection SET deck = \"\", deck_side = ? WHERE id_coll = ?""",
                            (
                                deck_name,
                                id_coll,
                            ),
                    ...
                    for card_data_deck in self.mainstore:
                        if (
                            ...
                            proxies_list_to_change.append(
                                [id_db, card_data_deck[15] * -1, 0]
                            )
                            proxies_list_to_change.append(
                                [id_db, card_data_deck[15], 1]
                            )
            elif current_side == 1:
                if proxy == 0:
                    ...
                    for id_coll in ids_coll_list:
                        c_coll.execute(
                            """UPDATE collection SET deck_side = \"\", deck = ? WHERE id_coll = ?""",
                            (
                                deck_name,
                                id_coll,
                            ),
                            ...
                            for i, card_data_deck in enumerate(self.mainstore):
                                if (
                                    ...
                                    if a_row_already_exists:
                                        ...
                                        card_data_deck[17] = 0
                                        card_data_deck[1] = card_data_deck[1][
                                            1:-1
                                        ].replace(defs.STRINGS["decks_sideboard"], "")
                                        card_data_deck[3] = card_data_deck[3][
                                            1:-1
                                        ].replace(defs.STRINGS["decks_sideboard"], "")
                    ...
                    for card_data_deck in self.mainstore:
                        if (
                            ...
                            proxies_list_to_change.append(
                                [id_db, card_data_deck[15] * -1, 1]
                            )
                            proxies_list_to_change.append(
                                [id_db, card_data_deck[15], 0]
                            )
    ...
    def delete_deck(self, deck_name):
        ...
        conn_coll, c_coll = functions.collection.connect_db()
        c_coll.execute(
            """SELECT id_coll, id_card FROM collection WHERE deck = ? OR deck_side = ?""",
            (
                deck_name,
                deck_name,
            ),
        ...
        c_coll.execute(
            """UPDATE collection SET deck = \"\", deck_side = \"\" WHERE deck = ? OR deck_side = ?""",
            (
                deck_name,
                deck_name,
            ),
        ...
        for card_data in responses:
            id_coll, id_card, deck, deck_side = card_data
            if (deck != "" and deck != deck_name) or (
                deck_side != "" and deck_side != deck_name
            ):
                in_deck = True
            else:
                in_deck = False
    ...
    def update_nb_decks(self):
        ...
        if self.label_nb_decks != None:
            ...
            functions.collection.disconnect_db(conn_coll)
            self.label_nb_decks.set_text(
                defs.STRINGS["list_decks_nb"].replace("%%%", str(count_nb))
            )
            return count_nb
        else:
            return None
    ...
    def gen_list_decks(self, deck_to_select):
        ...
        if self.store_list_decks != None:
            ...
            for id_deck, name, comment in responses:
                self.store_list_decks.append(
                    [str(id_deck), name, comment.replace("\n", " ")]
                )
    ...
    def send_id_to_loader(self, selection, integer, TreeViewColumn, simple_search):
        ...
        if pathlist != []:
            ...
            if nb_row_in_side > 0 and nb_row_not_in_side > 0:
                ...
                self.button_side.set_popover(
                    functions.decks.gen_sideboard_popover(
                        self,
                        self.button_side,
                        selection,
                        nb_row_in_side,
            ...
            self.button_move.set_popover(
                functions.decks.gen_move_deck_popover(self.button_move, selection, self)
            )
            self.button_move.set_sensitive(True)

            if len(pathlist) == 1:
                self.button_change_quantity.set_sensitive(True)
                self.button_change_quantity.set_popover(
                    functions.decks.gen_deck_change_quantity_popover(
                        self.button_change_quantity, selection, self
                    )
                )
            else:
