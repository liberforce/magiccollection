...
class Collection:
    ...
    def __init__(self, mainwindow):
        ...
        mainwindow.main_stack.add_titled(
            self.mainbox, "collection", defs.STRINGS["collection"]
        )
        ...
        if os.path.isfile(os.path.join(defs.HOMEMC, "collection.sqlite")):
            ...
            label_error_coll.set_markup(
                "<big>"
                + defs.STRINGS["db_coll_error"]
                .replace("&", "&amp;")
                .replace("<", "&lt;")
                .replace(">", "&gt;")
                + "</big>"
    ...
    def add_collection(self, cards_list, spinner_labels):
        ...
        def select_rows(rows_to_select):
            ...
            for po, nb_row in enumerate(rows_to_select):
                ...
                self.select.select_path(
                    nb_row
                )  # FIXME: this freezes GTK after selecting rows, why ??
        ...
        for card_to_add in cards_list:
            ...
            if card_found == 0:
                print(
                    "Something is wrong in the universe. In fact, only in the database but..."
                )
            else:
                (
                    id_,
                ...
                if self.mainstore is None:
                    ...
                    for card in cards.values():
                        cards_data_for_update_store_as.append(
                            [card["name"], card["edition_ln"]]
                        )
                else:
                    try:
                        if self.tree_coll.get_model() == self.mainstore:
                            GLib.idle_add(
                                self.label_nb_card_coll.set_text,
                                defs.STRINGS["nb_card_coll_s"].replace("%%%", "--"),
                            )
                    except:
                        pass
                    ...
                    if self.tree_coll.get_model() == self.searchstore:
                        ...
                        for j, row in enumerate(self.searchstore):
                            ...
                            if id_ in cur_ids:
                                ...
                                self.searchstore[j][15] = int(
                                    self.searchstore[j][15]
                                ) + int(nb)
                                if comment != "":
                                    # we need to bold the row
        ...
        if cards_data_for_update_store_as != []:
            GLib.idle_add(
                defs.MAINWINDOW.advancedsearch.update_current_store_bold,
                cards_data_for_update_store_as,
            )
        ...
        if self.tree_coll.get_model() == self.mainstore:
            ...
            if count_nb == 1:
                GLib.idle_add(
                    self.label_nb_card_coll.set_text,
                    defs.STRINGS["nb_card_coll"].replace("%%%", str(count_nb)),
                )
            else:
                GLib.idle_add(
                    self.label_nb_card_coll.set_text,
                    defs.STRINGS["nb_card_coll_s"].replace("%%%", str(count_nb)),
                )
    ...
    def del_collection(self, cards_to_delete):
        ...
        c_tmp.execute(
            """SELECT id_card, comment FROM collection WHERE id_card IN ("""
            + ids_list
            + """)"""
        )
        ...
        for card_coll in reponses_coll:
            ...
            try:
                ...
                if dict_responses_coll[id_card][1]:
                    dict_responses_coll[id_card] = [
                        dict_responses_coll[id_card][0] + 1,
                        True,
                    ]
                else:
                    if comment != "":
    ...
    def update_details(self, cards_to_update, new_id_db_to_bold, new_id_db_to_unbold):
        ...
        for id_coll, new_data in cards_to_update.items():
            ...
            c_coll.execute(
                """UPDATE collection SET condition = ?, lang = ?, foil = ?, loaned_to = ?, comment = ? WHERE id_coll = ?""",
                (
                    condition,
                    lang,
                    foil,
        ...
        if new_id_db_to_bold == "auto" and new_id_db_to_unbold == "auto":
            ...
            c.execute(
                """SELECT id_card, comment FROM collection WHERE id_coll IN ("""
                + ids_list
                + """)"""
            )
    ...
    def send_id_to_loader_with_selectinfo(
        self,
        selection,
        integer,
        TreeViewColumn,
        ...
        if pathlist == []:
            ...
            button_show_details.set_popover(
                functions.collection.gen_details_popover(
                    button_show_details, selection, self
                )
            )
            button_change_quantity.set_sensitive(True)
            button_change_quantity.set_popover(
                functions.collection.gen_quantity_popover(
                    button_change_quantity, selection
                )
            )
            ...
            if nb_avail > 0:
                ...
                button_add_deck.set_popover(
                    functions.collection.gen_add_deck_popover(
                        button_add_deck, selection
                    )
                )
            else:
                button_add_deck.set_sensitive(False)
        else:
            label_selectinfo.set_text(
                defs.STRINGS["info_selects_coll"].replace("%%%", str(len(pathlist)))
            )
        ...
        if selectinfo_button.get_active():
            ...
            if pathlist != []:
                ...
                for path in pathlist:
                    ...
                    name = (
                        model.get_value(tree_iter, column_name)
                        + " ("
                    ...
                    if len(pathlist) > 1:
                        ...
                        checkbutton.connect(
                            "toggled", checkbutton_toggled, path, selection
                        )
    ...
    def load_card_from_outside(
        self, widget_orig, cardid, list_widgets_to_destroy, simple_search
    ):
