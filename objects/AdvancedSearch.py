...
class AdvancedSearch:
    ...
    def __init__(self, mainwindow):
        ...
        mainwindow.main_stack.add_titled(
            self.mainbox, "advancedsearch", defs.STRINGS["advancedsearch"]
        )
        ...
        right_content_top_right = Gtk.Box(
        ...
        column_name_editions = Gtk.TreeViewColumn(
            defs.STRINGS["list_editions"], renderer_editions, text=0
        )
        ...
        select.connect(
            "changed",
            self.edition_selected,
            "blip",
            "blop",
            tree_editions,
            self.button_show_details,
        )
        ...
        right_content_top_left_bot = Gtk.Box(
        ...
        for comboboxtext in [
            ...
            for i in range(len(defs.SEARCH_ITEMS)):
                if i < 13:
                    comboboxtext.append(
                        defs.SEARCH_ITEMS[i][0], defs.SEARCH_ITEMS[i][1]
                    )
        ...
        for entry in [entry1, entry2, entry3, entry4]:
            ...
            entry.connect(
                ...
                overlay_right_content_bot,
                select,
            )
            entry.connect(
                "changed",
                self.update_button_search_and_reset,
                entry1,
                entry2,
                entry3,
                entry4,
        ...
        button_reset_search_pic = Gtk.Image.new_from_gicon(
            Gio.ThemedIcon(name="edit-clear-all-symbolic"), Gtk.IconSize.BUTTON
        )
        self.button_reset_search.add(button_reset_search_pic)
        self.button_reset_search.connect(
            "clicked", self.reset_search, entry1, entry2, entry3, entry4
        )
        ...
        label_cards_in_db.set_markup(
            "<big>"
            + defs.STRINGS["nb_cards_in_db"].replace("%%%", str(reponse[0]))
            + "</big>"
        )
    ...
    def disp_result2(self, reponses, type_s, wait_button):
        ...
        def _start(AS_object, store_results, scrolledwindow):
            ...
            AS_object.as_columns_list = functions.various.gen_treeview_columns(
                columns_to_display, tree_results
            )[0]
            ...
            if defs.OS == "mac":
                AS_object.mainstore.set_sort_func(
            ...
            tree_results.connect(
                "row-activated", self.show_details, select, self.button_show_details
            )
                        ...
                        try:
                            ...
                            try:
                                print_in_cards_added_reprints_release_date = int(
                                    cards[cards_added_reprints[unique_name]][
                                        "release_date"
                                    ]
                                )
                            except ValueError:
                            ...
                            if (
                                ...
                                > print_in_cards_added_reprints_release_date
                            ):
            ...
            if type_s == "edition":
                if nb_lines_added > 1:
                    self.label_nb_cards.set_text(
                        str(nb_lines_added) + " " + defs.STRINGS["cards"]
                    )
                else:
                    self.label_nb_cards.set_text(
                        str(nb_lines_added) + " " + defs.STRINGS["card"]
                    )
            else:
                if nb_lines_added > 1:
                    self.label_nb_cards.set_text(
                        str(nb_lines_added) + " " + defs.STRINGS["cards_result"]
                    )
                else:
                    self.label_nb_cards.set_text(
                        str(nb_lines_added) + " " + defs.STRINGS["card_result"]
                    )
    ...
    def prepare_request(
        self, widget, search_widgets_list, overlay_right_content_bot, select
    ):
        ...
        if defs.AS_LOCK == False:
            ...
            if request != None:
                ...
                GLib.idle_add(
                    self.icon_edition.set_from_gicon,
                ...
                try:
                    ...
                    thread = threading.Thread(
                        target=self.launch_ad_search, args=(request, "", wait_button)
                    )
                    thread.daemon = True
                    thread.start()
                else:
                    self.empty_box_results()
                    thread = threading.Thread(
                        target=self.disp_result, args=(prev_search, "", wait_button)
                    )
                    thread.daemon = True
                    thread.start()
    ...
    def edition_selected(
        self, selection, integer, TreeViewColumn, tree_editions, button_show_details
    ):
        ...
        if defs.AS_LOCK == False:
            ...
            if treeiter != None:
                ...
                request = (
                    """SELECT * FROM cards WHERE edition = \"""" + ed_code + """\""""
                )
                ...
                if os.path.isfile(
                    os.path.join(
                        defs.CACHEMCPIC,
                        "icons",
                        functions.various.valid_filename_os(ed_code) + ".png",
                    )
                ):
                    GLib.idle_add(
                        self.icon_edition.set_from_file,
                        os.path.join(
                            defs.CACHEMCPIC,
                            "icons",
                            functions.various.valid_filename_os(ed_code) + ".png",
                ...
                try:
                    ...
                    thread = threading.Thread(
                        target=self.launch_ad_search, args=(request, "edition", None)
                    )
                    thread.daemon = True
                    thread.start()
                else:
                    self.empty_box_results()
                    thread = threading.Thread(
                        target=self.disp_result, args=(prev_search, "edition", None)
                    )
                    thread.daemon = True
                    thread.start()
        ...
        if button.get_active():
            if name == "1":
                entry.set_icon_from_gicon(
                    Gtk.EntryIconPosition.PRIMARY, Gio.ThemedIcon(name="equal-symbolic")
                )
                entry.set_icon_tooltip_text(
                    Gtk.EntryIconPosition.PRIMARY, defs.STRINGS["entry_eq_ad"]
                )
            elif name == "2":
                entry.set_icon_from_gicon(
                    Gtk.EntryIconPosition.PRIMARY,
                    Gio.ThemedIcon(name="inf_or_eq-symbolic"),
                )
                entry.set_icon_tooltip_text(
                    Gtk.EntryIconPosition.PRIMARY, defs.STRINGS["entry_inf_eq_ad"]
                )
            elif name == "3":
                entry.set_icon_from_gicon(
                    Gtk.EntryIconPosition.PRIMARY,
                    Gio.ThemedIcon(name="sup_or_eq-symbolic"),
                )
                entry.set_icon_tooltip_text(
                    Gtk.EntryIconPosition.PRIMARY, defs.STRINGS["entry_sup_eq_ad"]
                )
            elif name == "4":
                entry.set_icon_from_gicon(
                    Gtk.EntryIconPosition.PRIMARY,
                    Gio.ThemedIcon(name="different-symbolic"),
                )
                entry.set_icon_tooltip_text(
                    Gtk.EntryIconPosition.PRIMARY, defs.STRINGS["entry_diff"]
                )
    ...
    def gen_list_editions(self):
        ...
        for info_edition in reponses:
            ...
            self.store_editions.append(
                [
                    nom_fr_ou_en,
                    info_edition[1],
                    info_edition[2],
                    info_edition[3].replace("-", ""),
                    info_edition[4],
                    info_edition[5],
                ]
            )
    ...
    def send_id_to_loader(self, selection, integer, TreeViewColumn, simple_search):
        ...
        if pathlist != []:
            ...
            if len(pathlist) == nb_row_in_coll:
                ...
                self.button_show_details.set_popover(
                    functions.collection.gen_details_popover(
                        self.button_show_details, selection, self
                    )
                )
            else:
                self.button_show_details.set_sensitive(False)
    ...
    def load_card(self, cardid, simple_search):
        ...
        GLib.idle_add(
            functions.cardviewer.gen_card_viewer,
            cardid,
            self.card_viewer,
            self,
            simple_search,
    ...
    def load_card_from_outside(
        self, widget, cardid, list_widgets_to_destroy, simple_search
    ):
