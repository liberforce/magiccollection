...
class MagicCollection(Gtk.Application):
    ...
    def do_startup(self):
        ...
        exportcollection_text_action = Gio.SimpleAction.new(
            "exportcollection_text", None
        )
    ...
    def about_cb(self, action, parameters, app):
        if self.aboutwindow is None:
            ...
            aboutdialog.set_title(
                defs.STRINGS["about"] + " - " + defs.STRINGS["app_name"]
            )
            ...
            aboutdialog.set_logo(
                functions.various.gdkpixbuf_new_from_file(
                    os.path.join(defs.PATH_MC, "images", "icons", "mclogo_min.png")
                )
            )
            ...
            aboutdialog.add_credit_section(
                defs.STRINGS["about_contributors"],
                defs.STRINGS["about_contributors_list"].split("&&&"),
            )
...
class MC_Window(Gtk.ApplicationWindow):
    ...
    def __init__(self, app):
        ...
        if last_width > 99 and last_height > 99:
            ...
            self.resize(
                defs.VARCONFIGDEFAULT["last_width"],
                defs.VARCONFIGDEFAULT["last_height"],
            )
        ...
        search_pic = Gtk.Image.new_from_gicon(
        ...
        search_button.connect(
            "clicked", self.search_button_toggled, search_entry, search_revealer
        )
    ...
    def search_launched(self, search_entry, main_stack):
        ...
        if search_text != "":
            ...
            if (
                search_text == "forest"
                or search_text == defs.STRINGS["forest"]
                or search_text == "island"
                or search_text == defs.STRINGS["island"]
                or search_text == "mountain"
                or search_text == defs.STRINGS["mountain"]
                or search_text == "swamp"
                or search_text == defs.STRINGS["swamp"]
                or search_text == "plains"
                or search_text == defs.STRINGS["plains"]
                ...
                if request is not None:
                    ...
                    try:
                        ...
                        thread = threading.Thread(
                            ...
                            args=(request, spinner, search_entry, current_object_view),
                        )
                        thread.daemon = True
                        thread.start()
                    else:
                        self.show_result(
                            prev_search, spinner, search_entry, current_object_view
                        )
    ...
    def search_by_name(self, request, spinner, search_entry, current_object_view):
        ...
        GLib.idle_add(
            self.show_result, reponses, spinner, search_entry, current_object_view
        )
    ...
    def show_result(self, reponses, spinner, search_entry, current_object_view):
        ...
        if len(reponses) == 0:
            ...
            result_window, nb_result_disp, store_results = (
                functions.various.create_window_search_name(
                    reponses, current_object_view
                )
            )
            if nb_result_disp == 1:
