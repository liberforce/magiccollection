...
def gen_decks_display(decks_object, box):
    ...
    if nb_decks < 1:
        ...
        label_welcome.set_markup(
            "<big>" + defs.STRINGS["decks_empty_welcome"] + "</big>"
        )
        ...
        image_button_show_details = Gtk.Image.new_from_gicon(
        ...
        right_content_top_left = Gtk.Box(
        ...
        column_name_decks = Gtk.TreeViewColumn(
            defs.STRINGS["list_decks_nb"], renderer_decks, text=1
        )
        ...
        column_comment_decks = Gtk.TreeViewColumn(
            defs.STRINGS["comment_deck"], renderer_comment, text=2
        )
        column_comment_decks.set_sort_column_id(2)
        tree_decks.append_column(column_comment_decks)

        if defs.OS == "mac":
            decks_object.store_list_decks.set_sort_func(
                1, functions.various.compare_str_osx, None
            )
            decks_object.store_list_decks.set_sort_func(
                2, functions.various.compare_str_osx, None
            )
        ...
        button_new_deck.add(
            Gtk.Image.new_from_gicon(
                Gio.ThemedIcon(name="document-new-symbolic"), Gtk.IconSize.BUTTON
            )
        )

        decks_object.button_change_comm_deck = Gtk.MenuButton()
        decks_object.button_change_comm_deck.set_popover(
            gen_edit_comm_name_deck_popover(
                decks_object.button_change_comm_deck,
                decks_object,
                decks_object.select_list_decks,
            )
        )
        decks_object.button_change_comm_deck.set_tooltip_text(
            defs.STRINGS["edit_comment_name_deck"]
        )
        decks_object.button_change_comm_deck.add(
            Gtk.Image.new_from_gicon(
                Gio.ThemedIcon(name="document-edit-symbolic"), Gtk.IconSize.BUTTON
            )
        )
        ...
        decks_object.button_estimate_deck.connect(
            "clicked", prepare_estimate_deck, decks_object.select_list_decks
        )
        decks_object.button_estimate_deck.set_tooltip_text(
            defs.STRINGS["estimate_deck"]
        )
        decks_object.button_estimate_deck.add(
            Gtk.Image.new_from_gicon(
        ...
        button_delete_deck.connect(
            "clicked", prepare_delete_deck, decks_object.select_list_decks, decks_object
        )
        ...
        for button in [
            button_new_deck,
            decks_object.button_change_comm_deck,
            decks_object.button_estimate_deck,
            button_delete_deck,
        ]:
            box_buttons.pack_start(button, False, True, 0)
        right_content_top_left.pack_start(box_buttons, False, True, 0)

        decks_object.select_list_decks.connect(
            "changed",
            decks_object.display_deck_content,
            "blip",
            "blop",
            tree_decks,
            button_delete_deck,
        ...
        label_click_deck.set_markup(
            "<big>" + defs.STRINGS["decks_click_deck"] + "</big>"
        )
        ...
        decks_object.right_content.pack_start(
            decks_object.right_content_bot, True, True, 0
        )
...
def prepare_estimate_deck(button, select_list_decks):
    ...
    request = (
        """SELECT DISTINCT id_card FROM collection WHERE deck = \""""
        + deck_name
        + """\" OR deck_side = \""""
        + deck_name
        + """\""""
...
def prepare_delete_deck(button, select_list_decks, decks_object):
    ...
    dialog = Gtk.MessageDialog(
        ...
        defs.STRINGS["delete_deck_warning"].replace("%%%", deck_name),
    )
    response = dialog.run()
    dialog.destroy()
    # -8 yes, -9 no
...
def prepare_delete_from_deck(button, deck_name, selection, decks_object):
    ...
    for row in pathlist:
        if model[row][16] == 0:
            ...
            ids_db_list_proxies_to_delete.append(
                [model[row][0], model[row][15] * -1, model[row][17]]
            )
    ...
    if len(ids_db_list_proxies_to_delete) > 0:
        GLib.idle_add(
            decks_object.change_nb_proxies, deck_name, ids_db_list_proxies_to_delete
        )
...
def textview_comment_save(textbuffer, decks_object, textview_comm):
    if textview_comm.get_sensitive():
        thread = threading.Thread(
            target=prepare_deck_comment_save, args=(textbuffer, decks_object)
        )
        thread.daemon = True
        thread.start()
...
def gen_deck_content(deck_name, box, decks_object):
    ...
    def _gen_complete_list_of_ids(
        dict_cards_in_deck, dict_proxies_in_deck, responses_proxies
    ):
    ...
    def _add_to_store(
        cards, side, dict_cards_in_deck, dict_proxies_in_deck, decksstore
    ):
        for id_, card in cards.items():
            if id_ in dict_cards_in_deck.keys():
                ...
                decksstore.insert_with_valuesv(
                    -1,
                    range(20),
                    [
                        card["id_"],
    ...
    conn, c = functions.collection.connect_db()
    c.execute(
        """SELECT id_coll, id_card FROM collection WHERE deck = ?""", (deck_name,)
    )
    responses = c.fetchall()
    c.execute(
        """SELECT id_coll, id_card FROM collection WHERE deck_side = ?""", (deck_name,)
    )
    responses_side = c.fetchall()
    c.execute(
        """SELECT proxies, proxies_side FROM decks WHERE name = ?""", (deck_name,)
    )
    ...
    decks_object.button_show_details = Gtk.MenuButton()
    decks_object.button_show_details.set_tooltip_text(
        defs.STRINGS["show_details_tooltip"]
    )
    decks_object.button_show_details.add(
        Gtk.Image.new_from_gicon(
            Gio.ThemedIcon(name="text-editor-symbolic"), Gtk.IconSize.BUTTON
        )
    )
    decks_object.delete_button = Gtk.Button()
    decks_object.delete_button.set_tooltip_text(
        defs.STRINGS["delete_from_deck_tooltip"]
    )
    decks_object.delete_button.add(
        Gtk.Image.new_from_gicon(
            Gio.ThemedIcon(name="deck_delete-symbolic"), Gtk.IconSize.BUTTON
        )
    )
    decks_object.button_change_quantity = Gtk.MenuButton()
    decks_object.button_change_quantity.set_tooltip_text(
        defs.STRINGS["change_quantity_tooltip"]
    )
    decks_object.button_change_quantity.add(
        Gtk.Image.new_from_gicon(
            Gio.ThemedIcon(name="zoom-in-symbolic"), Gtk.IconSize.BUTTON
        )
    )
    decks_object.button_move = Gtk.MenuButton()
    decks_object.button_move.set_tooltip_text(
        defs.STRINGS["move_to_other_deck_tooltip"]
    )
    decks_object.button_move.add(
        Gtk.Image.new_from_gicon(
            Gio.ThemedIcon(name="send-to-symbolic"), Gtk.IconSize.BUTTON
        )
    )

    decks_object.button_side = Gtk.MenuButton()
    decks_object.button_side.set_tooltip_text(defs.STRINGS["sideboard_tooltip"])
    decks_object.button_side.add(
        Gtk.Image.new_from_gicon(
            Gio.ThemedIcon(name="application-x-addon-symbolic"), Gtk.IconSize.BUTTON
        )
    )

    for button in [
        decks_object.button_show_details,
        decks_object.delete_button,
        decks_object.button_change_quantity,
        decks_object.button_move,
        decks_object.button_side,
    ]:
    ...
    dict_proxies_in_deck, nb_proxies_total = _proxies_from_deck_to_dict(
        responses_datadeck[0]
    )
    nb_cards = nb_cards + nb_proxies_total

    # the proxies in the sideboard of the deck
    dict_proxies_in_deck_side, nb_proxies_side_total = _proxies_from_deck_to_dict(
        responses_datadeck[1]
    )
    nb_cards_side = nb_cards_side + nb_proxies_side_total

    # the ids of the cards and the proxies in the deck (not in the sideboard)
    complete_list_of_ids = _gen_complete_list_of_ids(
        dict_cards_in_deck, dict_proxies_in_deck, responses_datadeck[0]
    )

    # the ids of the cards and the proxies in the sideboard of the deck
    complete_list_of_ids_side = _gen_complete_list_of_ids(
        dict_cards_in_deck_side, dict_proxies_in_deck_side, responses_datadeck[1]
    )

    if nb_cards < 2:
        decks_object.label_nb_cards.set_text(
            defs.STRINGS["nb_cards_in_deck"].replace("%%%", str(nb_cards))
        )
    else:
        decks_object.label_nb_cards.set_text(
            defs.STRINGS["nb_cards_in_deck_s"].replace("%%%", str(nb_cards))
        )

    if nb_cards_side > 0:
        tmp_text_cards = decks_object.label_nb_cards.get_text()
        tmp_text_side = defs.STRINGS["decks_in_sideboard"].replace(
            "%%%", str(nb_cards_side)
        )
        decks_object.label_nb_cards.set_text(
            defs.STRINGS["decks_order_cards_sideboard"]
            .replace("{cards}", tmp_text_cards)
            .replace("{sideboard}", tmp_text_side)
        )
    ...
    if nb_cards_side > 0:
        ...
        request_side = (
            """SELECT * FROM cards WHERE cards.id IN (""" + tmp_req_side + """)"""
        )
    ...
    decks_object.mainstore = Gtk.ListStore(
        str,
        str,
        str,
        str,
    ...
    decks_object.mainstore.set_sort_func(
    ...
    tree_deck.connect(
        "row-activated",
        decks_object.show_details,
        select,
        decks_object.button_show_details,
        decks_object.button_change_quantity,
    )
    decks_object.delete_button.connect(
        "clicked", prepare_delete_from_deck, deck_name, select, decks_object
    )
    ...
    _add_to_store(
        cards, 0, dict_cards_in_deck, dict_proxies_in_deck, decks_object.mainstore
    )
    ...
    if nb_cards_side > 0:
        ...
        _add_to_store(
            cards_side,
            1,
            dict_cards_in_deck_side,
            dict_proxies_in_deck_side,
            decks_object.mainstore,
        )
...
def prepare_move_cards(select_list_decks, selection, old_deck, decks_object):
    ...
    for row in pathlist:
        if model[row][16] == 0:  # not proxy
            ...
            proxies_list_to_change_remove.append(
                [model[row][0], model[row][15] * -1, model[row][17]]
            )
            proxies_list_to_change_add.append(
                [model[row][0], model[row][15], model[row][17]]
            )
    if len(ids_db_list) > 0:
        GLib.idle_add(decks_object.move_row, old_deck, new_deck, ids_db_list)
    if len(proxies_list_to_change_add) > 0:
        GLib.idle_add(
            decks_object.change_nb_proxies, old_deck, proxies_list_to_change_remove
        )
        GLib.idle_add(
            decks_object.change_nb_proxies, new_deck, proxies_list_to_change_add
        )


def prepare_add_cards_deck(current_deck_name, ids_coll_dict, decks_object, side):
    GLib.idle_add(
        decks_object.add_cards_to_deck, current_deck_name, ids_coll_dict, side
    )
...
def gen_deck_change_quantity_popover(button_change_quantity, selection, decks_object):
    ...
    def button_ok_clicked(
        ...
        if new_value > current_quantity:
            ...
            thread = threading.Thread(
                target=prepare_add_cards_deck,
                args=(current_deck_name, ids_coll_dict, decks_object, side),
            )
            thread.daemon = True
            thread.start()
            ...
            for card_id_coll in reversed(ids_card_current_to_use):
                ...
                if nb_added == nb_cards_to_delete:
                    break
            thread = threading.Thread(
                target=prepare_delete_cards_deck,
                args=(current_deck_name, ids_coll_dict, decks_object),
            )
            thread.daemon = True
            thread.start()
    ...
    def button_ok_clicked_proxies(
        ...
        GLib.idle_add(
            decks_object.change_nb_proxies, current_deck_name, proxies_list_to_change
        )
        popover.hide()

    def popover_show(
        popover, button_change_quantity, selection, quantity_box, decks_object
    ):
        ...
        if proxy == 0:
            details_store = functions.collection.gen_details_store(
                selection, decks_object
            )
            ids_cards_free_list = []
            ids_cards_in_current_deck_list = []
            ids_cards_in_current_deck_side_list = []
...
def gen_move_deck_popover(button_move, selection, decks_object):
    ...
    def row_activated(
        a, b, c, popover, select_list_decks, selection, current_deck_name, decks_object
    ):
        move_cards(
            None, popover, select_list_decks, selection, current_deck_name, decks_object
        )
    ...
    def move_cards(
        button, popover, select_list_decks, selection, old_deck, decks_object
    ):
        thread = threading.Thread(
            target=prepare_move_cards,
            args=(select_list_decks, selection, old_deck, decks_object),
        )
        thread.daemon = True
        thread.start()
        popover.hide()
...
def gen_edit_comm_name_deck_popover(
    button_change_comm_deck, decks_object, select_list_decks
):
    ...
    def popover_show(popover, decks_object, select_list_decks):
        def real_show(popover, decks_object, select_list_decks):
            def rename_deck(
                button, entry_name_deck, decks_object, popover, current_deck_name
            ):
                def real_rename_deck(
                    button, entry_name_deck, decks_object, popover, current_deck_name
                ):
                    popover.hide()
                    decks_object.rename_deck(
                        current_deck_name, entry_name_deck.get_text()
                    )
            ...
            c_coll.execute(
                """SELECT comment FROM decks WHERE name = \"""" + deck_name + """\""""
            )
            ...
            entry_name_deck.connect(
                "changed", entry_changed, ok_button, list_decks_names
            )
            ok_button.set_sensitive(False)
            ok_button.connect(
                "clicked",
            ...
            textbuffer.connect(
                "changed", textview_comment_save, decks_object, textview_comm
            )
...
def gen_new_deck_popover(button_new_deck, decks_object):
    ...
    def popover_show(popover, decks_object):
        def create_deck(button, entry_name_deck, decks_object, popover):
            ...
            GLib.idle_add(
                real_create_deck, button, entry_name_deck, decks_object, popover
            )
        ...
        def real_show(popover, decks_object):
            ...
            ok_button.set_sensitive(False)
            ok_button.connect(
                "clicked", create_deck, entry_name_deck, decks_object, popover
            )
            for widget in [label_name_deck, entry_name_deck, ok_button]:
...
def gen_sideboard_popover(
    decks_object, button_side, selection, nb_row_in_side, nb_row_not_in_side
):
    ...
    def popover_show(
        ...
        def button_sideboard_clicked(
            button, deck_name, ids_db_list, popover, decks_object
        ):
            ...
            GLib.idle_add(
                switch_sideboard, deck_name, ids_db_list, popover, decks_object
            )
        ...
        button_add_sideboard.connect(
            "clicked",
            button_sideboard_clicked,
            deck_name,
            ids_db_list,
            popover,
            decks_object,
        ...
        button_remove_sideboard.connect(
            "clicked",
            button_sideboard_clicked,
            deck_name,
            ids_db_list,
...
def update_comment_deck_to_db(decks_object, deck_name, new_comment):
    ...
    functions.various.lock_db(True, None)
    c_coll.execute(
        """UPDATE decks SET comment = ? WHERE name = ?""",
        (
            new_comment,
            deck_name,
        ),
