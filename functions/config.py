...
def change_config(param, value):
    ...
    if param in defs.VARCONFIGDEFAULT.keys():
        ...
        if currentvalue != value:
            ...
            lines = open(
                os.path.join(defs.CONFIGMC, "config"), "r", encoding="UTF-8"
            ).readlines()
...
def gen_warning_pic():
    pic = Gtk.Image.new_from_gicon(
        Gio.ThemedIcon(name="dialog-warning-symbolic"),
        Gtk.icon_size_from_name("12_config_warning"),
    )
...
def entry_dv_lang_changed(entry):
    ...
    if entry.get_sensitive():
        thread = threading.Thread(
            target=prepare_update_dvlang, args=([entry.get_text()])
        )
        thread.daemon = True
        thread.start()
...
def show_pref_dialog():
    ...
    if not defs.PREF_WINDOW_OPEN:
        ...
        if defs.LANGUAGE == "fr":
            ...
            checkbutton_ext_fr_name = Gtk.CheckButton(
                defs.STRINGS["config_ext_fr_name"]
            )
            ...
            checkbutton_ext_fr_name.connect(
                "toggled", checkbutton_toggled, "ext_fr_name"
            )
        ...
        if defs.LANGUAGE != "en":
            ...
            label_cardviewer.set_markup(
                "<b>" + defs.STRINGS["config_cardviewer"] + "</b>"
            )
            label_cardviewer.set_alignment(0.0, 0.5)
            box_display.pack_start(label_cardviewer, False, True, 0)

            checkbutton_show_en_name_in_card_viewer = Gtk.CheckButton(
                defs.STRINGS["config_show_en_name_in_card_viewer"]
            )
            ...
            checkbutton_show_en_name_in_card_viewer.connect(
                "toggled", checkbutton_toggled, "show_en_name_in_card_viewer"
            )
            box_display.pack_start(
                checkbutton_show_en_name_in_card_viewer, False, True, 0
            )
        ...
        checkbutton_add_collection_show_details = Gtk.CheckButton(
            defs.STRINGS["config_add_collection_show_details"]
        )
        if dict_config["add_collection_show_details"] == "1":
            checkbutton_add_collection_show_details.set_active(True)
        checkbutton_add_collection_show_details.connect(
            "toggled", checkbutton_toggled, "add_collection_show_details"
        )
        ...
        if Gtk.Settings.get_default().get_property("gtk-theme-name") == "Adwaita":
            ...
            label_general_aspect.set_markup(
                "<b>" + defs.STRINGS["config_general_aspect"] + "</b>"
            )
            ...
            checkbutton_dark_theme.connect(
                "toggled", checkbutton_dark_theme_toggled, "dark_theme"
            )
        ...
        label_nonenglish_names.set_markup(
            "<b>" + defs.STRINGS["config_nonenglish_names"] + "</b>"
        )
        ...
        box_columns_order_disp = Gtk.Box(
            orientation=Gtk.Orientation.HORIZONTAL, spacing=6
        )
        label_columns_order_disp = Gtk.Label()
        label_columns_order_disp.set_markup(
            "<b>" + defs.STRINGS["config_columns_order_disp"] + "</b>"
        )
        ...
        label_columns_order_disp_helper.set_markup(
            "<i><small>" + defs.STRINGS["config_columns_helper"] + "</small></i>"
        )
        box_columns.pack_start(label_columns_order_disp_helper, False, True, 0)

        box_columns_coll_decks = Gtk.Box(
            orientation=Gtk.Orientation.HORIZONTAL, spacing=6
        )
        scrolledwindow_columns_collection = gen_columns_choice(
            dict_config["coll_columns"].split(";"),
            defs.COLL_COLUMNS_CHOICE,
            "coll_columns",
            defs.STRINGS["config_columns_collection"],
        )
        box_columns_coll_decks.pack_start(
            scrolledwindow_columns_collection, True, True, 0
        )
        scrolledwindow_columns_decks = gen_columns_choice(
            dict_config["decks_columns"].split(";"),
            defs.DECKS_COLUMNS_CHOICE,
            "decks_columns",
            defs.STRINGS["config_columns_decks"],
        )
        ...
        scrolledwindow_columns_as = gen_columns_choice(
            dict_config["as_columns"].split(";"),
            defs.AS_COLUMNS_CHOICE,
            "as_columns",
            defs.STRINGS["config_columns_as"],
        )
        ...
        label_dv_add_to_collection.set_markup(
            "<b>" + defs.STRINGS["config_defaultvalues_addcoll_details"] + "</b>"
        )
        ...
        checkbutton_dv_condition = Gtk.CheckButton(
            defs.STRINGS["config_defaultvalues_condition"]
        )
        ...
        if dict_config["default_condition"] != "0":
            ...
            comboboxtext_dv_condition.set_active(
                int(tmp_dict_cond[dict_config["default_condition"]])
            )
        else:
            comboboxtext_dv_condition.set_sensitive(False)
        checkbutton_dv_condition.connect(
            "toggled",
            checkbutton_dv_condition_toggled,
            "default_condition",
            comboboxtext_dv_condition,
        )
        ...
        if dict_config["default_lang"] != "0":
            ...
            entry_dv_lang.set_sensitive(False)
        checkbutton_dv_lang.connect(
            "toggled", checkbutton_dv_lang_toggled, "default_lang", entry_dv_lang
        )
        ...
        checkbutton_download_pic_collection_decks = Gtk.CheckButton(
            defs.STRINGS["config_download_pic_collection_decks"]
        )
        if dict_config["download_pic_collection_decks"] == "1":
            checkbutton_download_pic_collection_decks.set_active(True)
        checkbutton_download_pic_collection_decks.connect(
            "toggled", checkbutton_toggled, "download_pic_collection_decks"
        )
        box_internet.pack_start(
            checkbutton_download_pic_collection_decks, False, True, 0
        )

        checkbutton_download_pic_as = Gtk.CheckButton(
            defs.STRINGS["config_download_pic_as"]
        )
        if dict_config["download_pic_as"] == "1":
            checkbutton_download_pic_as.set_active(True)
        checkbutton_download_pic_as.connect(
            "toggled", checkbutton_toggled, "download_pic_as"
        )
        ...
        checkbutton_not_internet_popup = Gtk.CheckButton(
            defs.STRINGS["config_not_internet_popup"]
        )
        if dict_config["not_internet_popup"] == "0":
            checkbutton_not_internet_popup.set_active(True)
        checkbutton_not_internet_popup.connect(
            "toggled", checkbutton_not_internet_popup_toggled, "not_internet_popup"
        )
        ...
        label_pic_cards_downloaded.set_markup(
            "<b>" + defs.STRINGS["config_pic_cards_downloaded"] + "</b>"
        )
        box_pic_cards.pack_start(label_pic_cards_downloaded, False, True, 0)

        box_pic_cards_downloaded_content = Gtk.Box(
        ...
        for label in [
            ...
            label_pics_cards,
            label_connection,
            label_nonenglish_names,
            label_columns_order_disp,
            label_columns_order_disp_helper,
            label_dv_add_to_collection,
            label_pic_cards_downloaded,
        ]:
            label.set_alignment(0.0, 0.5)
        for widget in [
            label_ext_sort_as,
            checkbutton_no_reprints,
            checkbutton_add_collection_show_details,
            checkbutton_download_pic_collection_decks,
            checkbutton_download_pic_as,
            checkbutton_not_internet_popup,
            label_fr_language,
            label_columns_order_disp_helper,
            box_columns_coll_decks,
            scrolledwindow_columns_as,
            box_pic_cards_downloaded_content,
            box_dv_condition,
            box_dv_lang,
        ]:
        ...
        notebook.append_page(
            box_defaultvalues, Gtk.Label(defs.STRINGS["config_defaultvalues"])
        )
...
def GetHumanReadableSize(size, precision=2):
    ...
    suffixes = [
        defs.STRINGS["config_pic_cards_size_b"],
        defs.STRINGS["config_pic_cards_size_ko"],
        defs.STRINGS["config_pic_cards_size_mo"],
        defs.STRINGS["config_pic_cards_size_go"],
        defs.STRINGS["config_pic_cards_size_to"],
    ]
    suffixIndex = 0
    ...
    end_val = (
        defs.STRINGS["config_pic_cards_size_unit"]
        .replace("{VALUE}", "%.*f")
        .replace("{SIZE}", "%s")
    )
...
def gen_pic_cards_downloaded_content(box_pic_cards_downloaded_content):
    ...
    if len(edition_with_pics_list) < 3:
        label_pic_cards_downloaded_editions_intro = Gtk.Label(
            defs.STRINGS["config_pic_cards_downloaded_editions_intro_none"]
        )
        box_pic_cards_downloaded_content.pack_start(
            label_pic_cards_downloaded_editions_intro, False, True, 0
        )
        ...
        thread = threading.Thread(
            target=get_size_pic_cache, args=[label_pic_cards_size]
        )
        thread.daemon = True
        thread.start()
        ...
        del_all_pics_ed_button.connect(
            "clicked", del_all_pics_ed_button_clicked, liststore
        )

        renderer_column_ed_name = Gtk.CellRendererText()
        column_column_ed_name = Gtk.TreeViewColumn(
            defs.STRINGS["config_pic_cards_downloaded_editions_intro"],
            renderer_column_ed_name,
            text=1,
        )
...
def gen_prices_box_content(box_prices, dict_config):
    ...
    if not prices_here:
        ...
        download_prices_button.connect(
            "clicked", down_prices_manual, box_prices, box_button, label, dict_config
        )
        box_prices.pack_start(download_prices_button, False, True, 0)
    else:
        label_prices_show = Gtk.Label()
        label_prices_show.set_markup(
            "<b>" + defs.STRINGS["config_prices_show"] + "</b>"
        )
        box_prices.pack_start(label_prices_show, False, True, 0)

        checkbutton_show_cards_prices = Gtk.CheckButton(
            defs.STRINGS["config_cardsprices_show"]
        )
        if dict_config["cards_price"] == "1":
            checkbutton_show_cards_prices.set_active(True)
        checkbutton_show_cards_prices.connect(
            "toggled", checkbutton_toggled, "cards_price"
        )
        ...
        label_prices_update.set_markup(
            "<b>" + defs.STRINGS["config_prices_update"] + "</b>"
        )
        box_prices.pack_start(label_prices_update, False, True, 0)

        checkbutton_prices_autoupdate = Gtk.CheckButton(
            defs.STRINGS["config_prices_autoupdate"]
        )
        if dict_config["price_autodownload"] == "1":
            checkbutton_prices_autoupdate.set_active(True)
        checkbutton_prices_autoupdate.connect(
            "toggled", checkbutton_toggled, "price_autodownload"
        )
        box_prices.pack_start(checkbutton_prices_autoupdate, False, True, 0)

        label_prices_last_update = Gtk.Label()
        label_prices_last_update.set_markup(
            defs.STRINGS["config_prices_version"].replace("%%%", defs.PRICES_DATE)
        )
        ...
        for widget in [
            checkbutton_show_cards_prices,
            box_prices_cur,
            checkbutton_prices_autoupdate,
        ]:
            widget.set_margin_left(12)


def gen_columns_choice(
    list_current_columns, list_all_columns, param_config, column_name
):
    ...
    column_checkbutton = Gtk.TreeViewColumn(
        defs.STRINGS["config_column_enabled"], renderer_checkbutton, active=1
    )
