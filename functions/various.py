    ...
    if check_internet():
        if os.path.isdir(os.path.join(defs.CACHEMCPIC, "icons")) == False:
            ...
            GLib.idle_add(
                ...
                "<b><big>" + defs.STRINGS["downloading_symbols"] + "</big></b>",
            )
            GLib.idle_add(functions.various.force_update_gui, 0)
            try:
                urllib.request.urlretrieve(
                    "http://mirandir.mcollection.free.fr/images/mc/symboles_editions/all.tar",
                    os.path.join(defs.CACHEMCPIC, "icons", "all.tar"),
                )
            except:
                pass
            else:
            ...
            if edition[1] == "1":
                if (
                    ...
                    GLib.idle_add(
                        defs.MAINWINDOW.widget_overlay.get_child().set_markup,
                        "<b><big>"
                        + defs.STRINGS["downloading_symbols"]
                        + " "
                        + str(i)
                        + "/"
                        + str(nbicontotal)
                    ...
                    GLib.idle_add(functions.various.force_update_gui, 0)
                    url_icon = (
                        "http://mirandir.mcollection.free.fr/images/mc/symboles_editions/"
                        + edition[0]
                        + ".png"
                    )
                    try:
...
def check_folders_config():
    ...
    if os.path.isfile(os.path.join(defs.CACHEMCPIC, "cardback.png")) == False:
        if check_internet():
            try:
                urllib.request.urlretrieve(
                    "http://mirandir.mcollection.free.fr/images/mc/cardback.png",
                    os.path.join(defs.CACHEMCPIC, "cardback.png"),
                )
            except:
                pass
...
def reporthook(block_no, block_size, file_size):
    ...
    GLib.idle_add(
        defs.MAINWINDOW.widget_overlay.get_child().set_markup,
        "<b><big>"
        + defs.STRINGS["downloading_db"]
        + " "
        + str(round(pourcentage))
        + "%</big></b>",
    )
...
def check_card_pic(code, name):
    ...
    if os.path.isfile(
        os.path.join(
            defs.CACHEMCPIC,
            valid_filename_os(code),
            valid_filename_os(name) + ".full.jpg",
        )
    ):
        return True
    else:
...
def gen_dict_editions():
    ...
    for info_edition in reponses:
        ...
        defs.DICT_EDITIONS[info_edition[0]] = [
            nom_fr_ou_en,
            info_edition[1],
            info_edition[3],
            info_edition[4],
...
def prepare_cards_data_for_treeview(cards):
    ...
    for card in cards:
        ...
        if card[16] != "":
            pix_colors = gdkpixbuf_new_from_file(
                os.path.join(
                    defs.PATH_MC, "images", "color_indicators", colors.lower() + ".png"
                )
            )
        else:
            pix_colors = gdkpixbuf_new_from_file(
...
def gen_treeview_columns(columns_to_display, treeview):
    ...
    def display_column(x, y, name, label, text, expand):
        ...
        column_name = Gtk.TreeViewColumn(
            label, renderer_text, text=text, weight=w, style=s
        )
    ...
    if "id_coll" in columns_to_display:
        ...
        column_id = Gtk.TreeViewColumn(
            defs.STRINGS["column_internal_id"],
            renderer_text_id,
            text=0,
            weight=w,
            style=s,
    ...
    if "colors" in columns_to_display:
        ...
        column_colors = Gtk.TreeViewColumn(
            defs.STRINGS["column_colors"], renderer_pixbuf_colors, pixbuf=5
        )
    ...
    if "power" in columns_to_display:
        pic_power = Gtk.Image.new_from_gicon(
            Gio.ThemedIcon(name="power-symbolic"), Gtk.IconSize.MENU
        )
        pic_power.show()
        column_power = display_column(
            10, 25, "power", defs.STRINGS["column_power"], 9, False
        )
        column_power.set_widget(pic_power)

    if "toughness" in columns_to_display:
        pic_toughness = Gtk.Image.new_from_gicon(
            Gio.ThemedIcon(name="toughness-symbolic"), Gtk.IconSize.MENU
        )
        pic_toughness.show()
        column_toughness = display_column(
            10, 25, "toughness", defs.STRINGS["column_toughness"], 10, False
        )
    ...
    if "coll_ed_nb" in columns_to_display:
        display_column(
            30, 25, "coll_ed_nb", defs.STRINGS["column_coll_ed_nb"], 18, False
        )
    ...
    if "price" in columns_to_display:
        ...
        if functions.prices.check_prices_presence():
            ...
            if price_cur == "0":
                ...
                label_col = "€"
        column_price = Gtk.TreeViewColumn(
            label_col, renderer_text_price, text=19, weight=w, style=s
        )
        dict_columns_list["price"] = column_price
        dict_renderers_list["price"] = renderer_text_price
        column_price.set_sort_column_id(19)
        column_price.set_cell_data_func(
            renderer_text_price,
            lambda col, cell, model, iter, unused: cell.set_property(
                "text", "{0}".format(truncate(model.get(iter, 19)[0]))
            ),
        )
...
def create_window_search_name(request_response, current_object_view):
    ...
    tree_results.connect(
        "row-activated", row_activated, select, current_object_view, window
    )
    ...
    if no_reprints == "1":
        ...
        for card in dict(cards).values():
            ...
            try:
                ...
                try:
                    print_in_cards_added_reprints_release_date = int(
                        cards[cards_added_reprints[unique_name]]["release_date"]
                    )
                except ValueError:
    ...
    for card in cards.values():
        ...
        if (
            card["name"] + "-" + card["nb_variant"] + "-" + card["edition_ln"]
            in cards_added
        ):
            add = False

        if add:
            store_results.insert_with_valuesv(
                -1,
                range(21),
                [
                    card["id_"],
    ...
    button_choose.connect(
        "clicked", button_choose_click, select, current_object_view, window
    )
...
def gen_details_widgets():
    ...
    grid_details.attach_next_to(
        comboboxtext_condition, label_add_condition, Gtk.PositionType.RIGHT, 1, 1
    )

    label_add_lang = Gtk.Label(defs.STRINGS["add_lang"])
    label_add_lang.props.halign = Gtk.Align.END
    grid_details.attach_next_to(
        label_add_lang, comboboxtext_condition, Gtk.PositionType.RIGHT, 1, 1
    )

    entry_lang = Gtk.Entry()
    grid_details.attach_next_to(
        entry_lang, label_add_lang, Gtk.PositionType.RIGHT, 1, 1
    )

    checkbutton_foil = Gtk.CheckButton(label=defs.STRINGS["add_foil"])
    grid_details.attach_next_to(
        checkbutton_foil, label_add_condition, Gtk.PositionType.BOTTOM, 2, 1
    )

    checkbutton_loaned = Gtk.CheckButton(label=defs.STRINGS["add_loaned"])
    checkbutton_loaned.props.halign = Gtk.Align.END
    grid_details.attach_next_to(
        checkbutton_loaned, checkbutton_foil, Gtk.PositionType.RIGHT, 1, 1
    )
    ...
    entry_loaned.set_sensitive(False)
    grid_details.attach_next_to(
        entry_loaned, checkbutton_loaned, Gtk.PositionType.RIGHT, 1, 1
    )
    ...
    grid_details.attach_next_to(
        label_add_comment, checkbutton_foil, Gtk.PositionType.BOTTOM, 4, 1
    )
    ...
    grid_details.attach_next_to(
        scrolledwindow, label_add_comment, Gtk.PositionType.BOTTOM, 4, 1
    )
...
def update_entrycompletions(entry_lang, entry_loaned):
    ...
    list_entrycompletion_lang = [
        defs.STRINGS["l_english"],
        defs.STRINGS["l_chinese"],
        defs.STRINGS["l_french"],
        defs.STRINGS["l_german"],
        defs.STRINGS["l_italian"],
        defs.STRINGS["l_japanese"],
        defs.STRINGS["l_korean"],
        defs.STRINGS["l_portuguese"],
        defs.STRINGS["l_russian"],
        defs.STRINGS["l_spanish"],
    ]
...
def clear_gui_del():
    ...
    try:
        defs.MAINWINDOW.collection.label_nb_card_coll.set_text(
            defs.STRINGS["nb_card_coll"].replace("%%%", "0")
        )
    except:
        pass
    try:
        if (
            defs.MAINWINDOW.collection.tree_coll.get_model()
            == defs.MAINWINDOW.collection.searchstore
        ):
...
def show_tips_window(mc):
    ...
    if mc.tips == None:
        ...
        notebook.append_page(
            scrolledwindow_general, Gtk.Label(defs.STRINGS["tips_general"])
        )
        ...
        for text in [
            defs.STRINGS["tips_search_text_partname"],
            defs.STRINGS["tips_search_text_nonenglishname"],
            defs.STRINGS["tips_search_text_logicop"],
            defs.STRINGS["tips_search_text_op"],
            defs.STRINGS["tips_search_text_null"],
            defs.STRINGS["tips_search_text_spetext"],
            defs.STRINGS["tips_search_text_rarity"],
            defs.STRINGS["tips_search_text_manacost"],
            defs.STRINGS["tips_search_text_colors"],
            defs.STRINGS["tips_search_text_condition"],
        ]:
            if text != "":
        ...
        notebook.append_page(
            scrolledwindow_search, Gtk.Label(defs.STRINGS["tips_search"])
        )
        ...
        notebook.append_page(
            scrolledwindow_proxies, Gtk.Label(defs.STRINGS["tips_proxies"])
        )
...
def show_shortcuts_window(mc):
    ...
    if mc.shortcutswindow == None:
        ...
        shortkey_collection_mode.props.title = defs.STRINGS[
            "shortcuts_switch_collection"
        ]
        ...
        for widget in [
            section,
            group,
            shortkey_simple_search,
            shortkey_collection_mode,
            shortkey_decks_mode,
            shortkey_as_mode,
        ]:
            widget.show()
